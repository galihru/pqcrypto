...
pub enum LaiCryptoError {
    NoSquareRootFound { x: u128, y: u128, s: u128, hh: u128 },
    MaxAttemptsExceeded,
}
...
/// One step transformation T (safe)
pub fn t(point: (u128, u128), s: u128, a: u128, p: u128) -> Result<(u128, u128), LaiCryptoError> {
    let (x, y) = point;
    ...
    Err(LaiCryptoError::MaxAttemptsExceeded)
}
...
pub fn keygen(
    ...
    a: u128,
    p0: (u128, u128),
) -> Result<(u128, (u128, u128)), LaiCryptoError> {
    for _ in 0..100 {
        let mut buf = [0u8; 16];
        OsRng.fill_bytes(&mut buf);
        let k = u128::from_be_bytes(buf) % (p - 1) + 1;
        if let Ok(q) = pow_t_range(p0, 1, k, a, p) {
            return Ok((k, q));
        }
    }
    Err(LaiCryptoError::MaxAttemptsExceeded)
...
pub fn encrypt(
    ...
    for _ in 0..100 {
        ...
        if let Ok(c1) = pow_t_range(p0, 1, r, a, p) {
            if let Ok(sr) = pow_t_range(public_q, 1, r, a, p) {
                let m_val = (m % p, 0);
